...
afterEach(async () => {
  // Remove temp dirs sequentially; swallow errors — partial cleanup is fine,
  // OS will reclaim eventually and cleanup should never fail the test.
...
describe("repoUsesJj", () => {
  test("detects jj in a colocated jj+git repo", async () => {
    const root = await makeTempDir();
    await mkdir(join(root, ".jj"));
    await mkdir(join(root, ".git"));

    expect(repoUsesJj(root)).toBe(true);
  });

  test("returns false for plain git repos", async () => {
    const root = await makeTempDir();
    await mkdir(join(root, ".git"));

    expect(repoUsesJj(root)).toBe(false);
  });
});
...
describe("fresh note helpers", () => {
  ...
  test("keeps bracket-prefixed body lines inside the same exact block", () => {
    const output = [
      "=== workflow context: integrations/pi/index.ts @ abc (mycelium) ===",
      "",
      "[exact] Active note (summary)",
      "kind summary",
      "",
      "Body before bracket line.",
      "[literal bracketed body line]",
      "Still the same note body.",
      "",
      "[tree] Parent dir note (context)",
      "kind context",
    ].join("\n");
